...
def check_leave_status(user_id: str) -> str:
    ...
    Data Returned (JSON): from_date, to_date, status, leave_type, destination, reason, reviewed_by.
    Use Cases: Use this when the user asks if their leave was approved, what their leave status is, or if they have active leaves.
    """
    try:
...
def get_student_profile(user_id: str) -> str:
    ...
    Data Returned (JSON): student_name, phone, hostel, room_number, curfew_violations, and guardian_contact_info.
    Use Cases: Use this when the user asks about their profile, hostel room, or emergency contacts. Use this proactively when the user is planning a Casual Outing.
    Constraints: You must check their curfew_violations. If they have 4 or more, warn them they must return before 10:30 PM.
    """
    try:
...
def draft_campus_leave_application(
    ...
    """Draft a new campus leave application for the user to review.
    Data Returned (String): A formatted draft message that you MUST show to the user.
    Use Cases: Use this FIRST whenever the user wants to apply for a formal leave.
    Constraints: NEVER use this tool for a Casual Outing like going to the mall. If missing required details (dates, destination, reason, emergency_contact), you MUST ask them. You MUST ask the user "Do you confirm this application?" after returning this draft. Do NOT submit it automatically.
    """
    try:
        profile = _profile_for(DEFAULT_CAMPUS_ID, user_id)
        if not profile:
...
def submit_campus_leave_application(
    ...
    """Submit the previously drafted campus leave application to the database.
    Data Returned (String): success confirmation message.
    Use Cases: Use this ONLY after the user explicitly replies 'Yes' or 'Confirm' to the draft generated by `draft_campus_leave_application`.
    Constraints: WARNING: You are STRICTLY FORBIDDEN from using this tool unless you have ALREADY generated a draft using `draft_campus_leave_application` AND the user explicitly replied 'Yes' to that draft.
    """
    try:
        profile = _profile_for(DEFAULT_CAMPUS_ID, user_id)
        if not profile:
